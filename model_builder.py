...
def suggest_performance(outputs,labels, display = 1):
    y_true = []
    y_pred = []
...
class model_build:
    history = []
    def __init__(self,model,data,nu_fold = 5, ratio = 0.2, shuff=True, r_state=42, criterion = nn.CrossEntropyLoss()):
        ...
        self.n_fold = nu_fold
        if self.n_fold > 1:
            splits = KFold(n_splits=nu_fold,shuffle=shuff,random_state=r_state)
        ...
        if self.n_fold > 1:  
            for fold, (train_idx,val_idx) in enumerate(self.fold):
                ...
                for epoch in range(num_epochs):
                    ...
                    #loss calculation
                    train_dataloader = torch.utils.data.DataLoader(self.train_data, batch_size=batch, sampler= train_idx)
            ...
            fmodel = self.model.to(device)
            optimizer = optim.Adam(fmodel.parameters(), lr=lrate)
            N = N = len(self.train_data)
            num_train = math.floor(0.8*N)
            num_test = N - num_train
            train_data, val_data = random_split(self.train_data, [num_train, num_test])
    ...
    def testing(self, performance = suggest_performance, visualize = 1):
        ...
        for fold_model in self.history:
            loss, acc = self.val_epoch(fold_model["model"], device, self.criterion, test_dataloader, performance, display = visualize)
            tem = {'model': c, 'loss': loss, 'acc': acc}
            result.append(tem)
            c = c+1
        return result